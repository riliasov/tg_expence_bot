...
Бот для учета расходов в Google Sheets.
...
1. Установите зависимости:
   ...
   ```
2. Создайте папку `secrets` и добавьте туда `.env` и файл ключей Google (`planeta_sheets_key.json`).
3. Запустите бота:
   ```bash
   python main.py
   ```
...
Используйте скрипт `fast_push.sh`:

```bash
./fast_push.sh "Описание изменений"
```

Это автоматически добавит все файлы, сделает коммит и отправит их в репозиторий.

### ☁️ Деплой в Google Cloud Run

1. Убедитесь, что у вас установлен `gcloud CLI`.
2. Отредактируйте `deploy.sh`, указав ваш `PROJECT_ID`.
3. Создайте секрет для ключей Google:
   ```bash
